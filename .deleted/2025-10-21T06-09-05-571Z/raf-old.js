...
const { getIntentFromGemini, getConversationalResponse } = require("../lib/gemini"); // Impor fungsi Gemini
...
module.exports = async(raf, msg, m) => {
    ...
    const chats = (type === 'conversation' && msg.message.conversation) ? msg.message.conversation : (type == 'imageMessage') && msg.message.imageMessage.caption ? msg.message.imageMessage.caption : (type == 'documentMessage') && msg.message.documentMessage.caption ? msg.message.documentMessage.caption : (type == 'videoMessage') && msg.message.videoMessage.caption ? msg.message.videoMessage.caption : (type == 'extendedTextMessage') && msg.message.extendedTextMessage.text ? msg.message.extendedTextMessage.text : (type == 'buttonsResponseMessage' && msg.message.buttonsResponseMessage.selectedButtonId) ? msg.message.buttonsResponseMessage.selectedButtonId : (type == 'templateButtonReplyMessage') && msg.message.templateButtonReplyMessage.selectedId ? msg.message.templateButtonReplyMessage.selectedId : msg.message?.interactiveResponseMessage?.nativeFlowResponseMessage ? (JSON.parse(msg.message.interactiveResponseMessage.nativeFlowResponseMessage?.paramsJson || '{}')?.id || '') : ""
    ...
    try {
        ...
        if (temp[sender]) {
            ...
            if (temp[sender].step === 'AWAITING_QUESTION') {
                ...
                // Anggap pesan berikutnya sebagai pertanyaan, langsung kirim ke AI percakapan
                await raf.sendPresenceUpdate('composing', from);
                try {
                    // `chats` berisi pertanyaan pengguna yang sebenarnya
                    const answer = await getConversationalResponse(chats); 
                    reply(answer);
                } catch (e) {
                    console.error("[AWAITING_QUESTION_ERROR]", e);
                    reply("Maaf, sepertinya AI sedang sibuk. Coba lagi beberapa saat ya.");
                } finally {
        ...
        switch (intent) {
            ...
            case 'TANYA_JAWAB_UMUM': {
                ...
                try {
                    ...
                    console.error("[TANYA_JAWAB_UMUM_ERROR]", e);
                    reply("Maaf, sepertinya AI sedang sibuk. Coba lagi beberapa saat ya.");
                } finally {
                    // Hentikan "sedang mengetik..."
                    await raf.sendPresenceUpdate('paused', from);
                }
                break;
            }
            ...
            break;
            case 'LAPOR_GANGGUAN': {
                // --- ALUR BARU: PERCAKAPAN, BUKAN LANGSUNG BUAT TIKET ---
                const pelangganId = sender; // JID lengkap pelanggan, misal: 628xxxx@s.whatsapp.net
                const pelangganPushName = pushname;
                const pelangganPlainNumber = pelangganId.split('@')[0]; // Nomor WA pelanggan saja, misal: 628xxxx

                // ===== VALIDASI 1: PELANGGAN HARUS TERDAFTAR =====
                const userPelanggan = global.users.find(u =>
                    u.phone_number &&
                    u.phone_number.split("|").some(num =>
                        num.trim() === pelangganPlainNumber ||
                        `62${num.trim().substring(1)}` === pelangganPlainNumber
                    )
                );

                if (!userPelanggan) {
                    // Jika nomor pengirim tidak ditemukan di database pelanggan
                    return reply(mess.userNotRegister); // mess.userNotRegister harusnya berisi: 'Anda tidak terdaftar...'
                }
                // ===== AKHIR VALIDASI 1 =====

                // ===== VALIDASI 2: CEK APAKAH PELANGGAN SUDAH PUNYA LAPORAN AKTIF =====
                const laporanAktifPelanggan = global.reports.find(
                    report => report.pelangganId === pelangganId &&
                              (report.status === 'baru' || report.status === 'diproses teknisi')
                );

                if (laporanAktifPelanggan) {
                    let statusText = laporanAktifPelanggan.status;
                    if (statusText === 'baru') {
                        statusText = 'Baru (menunggu diproses teknisi)';
                    } else if (statusText === 'diproses teknisi') {
                        statusText = `Sedang Diproses (oleh ${laporanAktifPelanggan.processedByTeknisiName || 'Teknisi'})`;
                    }
                    ...
                    const tanggalLaporanAktifDibuatFormatted = new Date(laporanAktifPelanggan.createdAt).toLocaleString('id-ID', optionsDateFormat);
                    const cuplikanLaporanAktif = laporanAktifPelanggan.laporanText.substring(0, 100) + (laporanAktifPelanggan.laporanText.length > 100 ? '...' : '');

                    // Pesan penolakan jika ada laporan aktif (sudah dibuat 1 baris dan informatif)
                    const pesanLaporanAktif = `⚠️ *Anda Masih Memiliki Laporan Aktif* ⚠️\n\nHalo ${pelangganPushName},\n\nKami informasikan bahwa Anda masih memiliki satu laporan yang sedang kami tangani dan belum selesai. Untuk memastikan setiap laporan ditangani dengan baik, kami mohon kesediaan Anda untuk menunggu hingga laporan tersebut terselesaikan sebelum membuat laporan baru.\n\nBerikut adalah rincian laporan Anda yang masih aktif:\n-----------------------------------\n*Nomor Tiket:* *${laporanAktifPelanggan.ticketId}*\n*Status Saat Ini:* ${statusText}\n*Dilaporkan Pada:* ${tanggalLaporanAktifDibuatFormatted}\n*Ringkasan Keluhan Anda:*\n"${cuplikanLaporanAktif}"\n-----------------------------------\n\nTim kami sedang bekerja untuk menyelesaikan laporan di atas. Anda akan menerima notifikasi otomatis untuk setiap perkembangan statusnya.\n\nJika laporan baru yang ingin Anda buat adalah untuk *masalah yang sama*, mohon untuk tidak membuat tiket baru. Jika ini adalah *masalah yang benar-benar berbeda*, mohon tunggu hingga tiket di atas selesai atau hubungi admin langsung jika bersifat sangat mendesak.\n\nTerima kasih atas pengertian dan kerjasamanya.\nTim Layanan ${global.config.nama || "Kami"}`; // Pastikan global.config.nama ada
                    return reply(pesanLaporanAktif);
                }
                // ===== AKHIR VALIDASI 2 =====

                // Jika validasi lolos, lanjutkan ke penanganan deskripsi
                let laporanText = entities.deskripsi_keluhan;

                // Jika intent berasal dari keyword 'lapor', ambil sisa teks sebagai deskripsi
                if (!laporanText && command === 'lapor') {
                    laporanText = q.trim();
                }

                // Jika deskripsi KOSONG (dari keyword handler)
                if (!laporanText || laporanText.trim() === "") {
                    // Tanya deskripsi dan set state untuk menunggu jawaban
                    temp[sender] = {
                        step: 'LAPOR_GANGGUAN_AWAITING_DESCRIPTION',
                        targetUser: userPelanggan
                    };
                    return reply("Saya mengerti Anda mengalami gangguan. Bisa tolong jelaskan lebih detail masalahnya seperti apa? (Contoh: 'internet mati total' atau 'lemot untuk streaming').");
                }

                // Jika deskripsi ADA (dari AI atau command `lapor ...`), langsung mulai alur percakapan normal
                temp[sender] = {
                    step: 'LAPOR_GANGGUAN_ASK_REBOOT',
                    original_complaint: laporanText,
                    targetUser: userPelanggan // Simpan data user untuk nanti
                };

                // Kirim pertanyaan pertama
                reply(`Saya turut prihatin dengan kendala Anda terkait "${laporanText}".\n\nSebelum saya buatkan laporan ke tim teknisi, apakah Anda sudah mencoba me-reboot modem terlebih dahulu?\n\nBalas *'ya'* atau *'tidak'*.`);
            }
            break;
            case 'CEK_TIKET': { // Sesuaikan dengan intent yang mungkin dari Gemini
                ...
                const namaPelanggan = report.pelangganPushName || (report.pelangganDataSystem ? report.pelangganDataSystem.name : "N/A");
                const layananPelanggan = (report.pelangganDataSystem && report.pelangganDataSystem.subscription) ? report.pelangganDataSystem.subscription : "Tidak terinfo";
                ...
                if (report.status === 'baru') {
                    statusDetail = `*Baru* (Menunggu antrian teknisi).\nEstimasi penanganan akan diinformasikan lebih lanjut.`;
                    pesanTambahan = `Mohon kesabarannya ya Kak, laporan Anda akan segera kami proses. 🙏`;
                } else if (report.status === 'diproses teknisi') {
                    const teknisiYangProses = report.processedByTeknisiName || "Teknisi Kami";
                    ...
                    const teknisiYangSelesaikan = report.resolvedByTeknisiName || "Teknisi Kami";
                    const waktuSelesai = report.resolvedAt ? new Date(report.resolvedAt).toLocaleString('id-ID', optionsDateFormat) : "N/A";
                    statusDetail = `*Selesai* ditangani oleh ${teknisiYangSelesaikan}.\n✅ *Diselesaikan Pada:* ${waktuSelesai}`;
                    pesanTambahan = `Semoga layanan kami kembali normal dan memuaskan. Jika masih ada kendala terkait laporan ini, silakan hubungi Admin.`;
                } else if (report.status === 'dibatalkan') {
            ...
            break;
            case 'SELESAIKAN_TIKET': // Sesuaikan dengan intent yang mungkin dari Gemini
            case 'tiketdone': {
                if (!isTeknisi && !isOwner) return reply(mess.teknisiOrOwnerOnly);
                if (!q) return reply("Mohon sertakan nomor tiket yang ingin diselesaikan. Contoh: selesaikantiket LP-xxxxxxxxxx-xxxx");

                const ticketIdToResolve = q.trim();
                const reportIndex = global.reports.findIndex(r => r.ticketId === ticketIdToResolve);

                if (reportIndex === -1) {
                    return reply(format('mess_reportNotFound_detail', { ticketId: ticketIdToResolve }));
                }

                const report = global.reports[reportIndex];

                if (report.status === 'selesai') {
                    return reply(format('mess_reportAlreadyDone_detail', { ticketId: ticketIdToResolve }));
                }

                report.status = 'selesai';
                report.resolvedAt = new Date().toISOString();
                report.resolvedByTeknisiId = sender; // JID teknisi/owner yang menyelesaikan
                report.resolvedByTeknisiName = isTeknisi ? isTeknisi.username : (isOwner ? "Owner" : "Sistem");


                global.reports[reportIndex] = report; // Update array global
                saveReportsToFile(global.reports); // Simpan ke reports.json

                await reply(`✅ Tiket *${ticketIdToResolve}* berhasil ditandai sebagai *SELESAI*.\nTerima kasih atas kerjasamanya.`);

                // Notifikasi ke pelanggan bahwa laporan sudah selesai
                if (report.pelangganId) {
                    ...
                    const messageToPelanggan = `✨ Halo ${namaPelangganUntukNotif},\n\nLaporan Anda dengan nomor tiket *${report.ticketId}* mengenai "${report.laporanText.substring(0, 50)}..." telah berhasil *SELESAI* ditangani oleh teknisi kami.\n\nTerima kasih telah menggunakan layanan ${nama}.\nJika ada kendala lain, jangan ragu untuk melapor kembali.`;
                    try {
                        await raf.sendMessage(report.pelangganId, { text: messageToPelanggan });
                    } catch (err) {
                        console.error(`Gagal mengirim notifikasi penyelesaian ke pelanggan ${report.pelangganId} untuk tiket ${report.ticketId}:`, err.message);
                        // Kirim info kegagalan ke teknisi jika perlu
                        await reply(`Info: Gagal mengirim notifikasi selesai ke pelanggan (${report.pelangganId.split('@')[0]}) untuk tiket ${report.ticketId}.`);
                    }
                }
            }
            break;
            ...
            break
            case 'button': {
                await sendButton(`Hi Kak ${pushname}\nSilahkan Klik Menu Yang Telah Disediakan.`, 'RAF NET BOT', [
                    {
                        type: "single_select",
                        content: {
                            title: 'MENU',
                            sections: [
                                {
                                    ...
                                    rows: [
                                        {
                                            header: "",
                                            title: 'MENU WIFI', description: "List Menu Wifi", id: `menuwifi` },
                                        {
                                            header: "",
                                            title: 'MENU PELANGGAN', description: "List Menu Pelanggan", id: `menupelanggan` },
                                        {
                                            header: "",
                                            title: 'PASANG', description: "Harga Pasang Wifi", id: `pasang` }
                                    ]
                                },
                            ]
                        }
