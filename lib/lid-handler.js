...
async function findUserWithLidSupport(users, msg, plainSenderNumber, raf = null) {
    ...
    // DEBUG: Log all available fields in msg.key for @lid
    if (senderInfo.isLid) {
        console.log('[LID_HANDLER] @lid detected, checking available fields:');
        console.log('[LID_HANDLER] - msg.key:', JSON.stringify(msg.key, null, 2));
        console.log('[LID_HANDLER] - msg.participant:', msg.participant);
        console.log('[LID_HANDLER] - senderInfo:', JSON.stringify(senderInfo, null, 2));
    }
    
    // If phone not found from message fields, try signal repository (Baileys v7)
    if (senderInfo.isLid && !senderInfo.phoneNumber && raf) {
        const lidJid = senderInfo.originalSender;
        
        // Method 1: Try signalRepository.lidMapping (Baileys v7 official method)
        if (raf.signalRepository) {
            try {
                // Try getPNForLID method
                if (raf.signalRepository.lidMapping && typeof raf.signalRepository.lidMapping.getPNForLID === 'function') {
                    const phoneNumber = await raf.signalRepository.lidMapping.getPNForLID(lidJid);
                    console.log(`[LID_HANDLER] signalRepository.lidMapping.getPNForLID result:`, phoneNumber);
                    if (phoneNumber) {
                        senderInfo.phoneNumber = phoneNumber.split('@')[0].split(':')[0];
                        senderInfo.method = 'signal_repository_lidMapping';
                    }
                }
                
                // Try getPhoneNumber method (alternative)
                if (!senderInfo.phoneNumber && typeof raf.signalRepository.getPhoneNumber === 'function') {
                    const phoneNumber = await raf.signalRepository.getPhoneNumber(lidJid);
                    console.log(`[LID_HANDLER] signalRepository.getPhoneNumber result:`, phoneNumber);
                    if (phoneNumber) {
                        senderInfo.phoneNumber = phoneNumber.split('@')[0].split(':')[0];
                        senderInfo.method = 'signal_repository_getPhoneNumber';
                    }
                }
            } catch (error) {
                console.log(`[LID_HANDLER] signalRepository method failed:`, error.message);
            }
        }
        
        // Method 2: Try authState.creds (contains lid-phone mappings)
        if (!senderInfo.phoneNumber && raf.authState && raf.authState.creds) {
            try {
                const creds = raf.authState.creds;
                // Check if there's a lid mapping in creds
                if (creds.lidTagHash || creds.pnForLid) {
                    console.log(`[LID_HANDLER] Found lid data in authState.creds`);
                }
            } catch (error) {
                console.log(`[LID_HANDLER] authState method failed:`, error.message);
            }
        }
        
        // Method 3: Try store.contacts (if available)
        if (!senderInfo.phoneNumber && raf.store && raf.store.contacts) {
            try {
                const contact = raf.store.contacts[lidJid];
                if (contact && contact.id && contact.id.includes('@s.whatsapp.net')) {
                    senderInfo.phoneNumber = contact.id.split('@')[0].split(':')[0];
                    senderInfo.method = 'store_contacts';
                    console.log(`[LID_HANDLER] Found phone from store.contacts:`, senderInfo.phoneNumber);
                }
            } catch (error) {
                console.log(`[LID_HANDLER] store.contacts method failed:`, error.message);
            }
        }
    }
    
    // Check stored mapping as fallback
    if (senderInfo.isLid && !senderInfo.phoneNumber) {
...
function createLidVerification(lidId, users) {
    return {
        ...
        message: `🔐 *VERIFIKASI AKUN DIPERLUKAN*\n\n` +
                 `WhatsApp Anda menggunakan format privacy baru yang menyembunyikan nomor telepon.\n\n` +
                 `Untuk melanjutkan, silakan ketik:\n` +
                 `*link [nomor_hp_terdaftar]*\n\n` +
                 `Contoh:\n` +
                 `• link 6285233047094\n` +
                 `• link 085233047094\n\n` +
                 `_Nomor HP harus sesuai dengan yang terdaftar di database._`
    };
}
