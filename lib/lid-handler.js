...
async function findUserWithLidSupport(users, msg, plainSenderNumber, raf = null) {
    ...
    // Log only essential info for @lid
    if (senderInfo.isLid) {
        console.log(`[LID_HANDLER] @lid detected, method: ${senderInfo.method}, phone: ${senderInfo.phoneNumber || 'not found'}`);
    }
    ...
    if (senderInfo.isLid && !senderInfo.phoneNumber && raf) {
            ...
            try {
                ...
                if (raf.signalRepository.lidMapping && typeof raf.signalRepository.lidMapping.getPNForLID === 'function') {
                    ...
                    if (phoneNumber) {
                        ...
                        console.log(`[LID_HANDLER] Found phone via signalRepository: ${senderInfo.phoneNumber}`);
                    }
                }
                ...
                if (!senderInfo.phoneNumber && typeof raf.signalRepository.getPhoneNumber === 'function') {
                    ...
                    if (phoneNumber) {
                        ...
                        console.log(`[LID_HANDLER] Found phone via getPhoneNumber: ${senderInfo.phoneNumber}`);
                    }
                }
            } catch (error) {
        ...
        // Method 2: Try store.contacts (if available)
