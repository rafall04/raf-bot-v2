    ...
    if (config.status_schedule === true && isValidCron(config.schedule)) {
        ...
        cronTaskReminder = cron.schedule(config.schedule, async () => {
            ...
            if (currentDay === reminderDay || skipDayCheck) {
                ...
                console.log(`[CRON_REMINDER] WhatsApp connected: ${global.conn ? 'YES' : 'NO'}`);
                ...
                for (let user of global.users) {
                    ...
                    if (!whitelistedProfile.includes(userProfile) && !user.paid) {
                        ...
                        if (!global.conn) {
                            console.error(`[CRON_REMINDER_ERROR] WhatsApp not connected! Cannot send messages.`);
                            break;
                        }
...
function initCompensationRevertTask(config) {
    ...
    if (config.status_compensation_revert === true && isValidCron(config.schedule_compensation_revert)) {
        console.log("[CRON_COMPENSATION] Starting/Restarting compensation revert task with schedule:", config.schedule_compensation_revert);
        cronTaskCompensationRevert = cron.schedule(config.schedule_compensation_revert, async () => {
            console.log("[CRON_COMPENSATION] Compensation revert task triggered at:", new Date().toISOString());
            ...
            console.log(`[CRON_COMPENSATION] Checking ${currentCompensationsData.length} compensations for expiry...`);
            const activeCompensations = currentCompensationsData.filter(c => c.status === 'active');
            console.log(`[CRON_COMPENSATION] Found ${activeCompensations.length} active compensations`);
            ...
            for (const comp of currentCompensationsData) {
                if (comp.status === 'active' && new Date(comp.endDate) <= now) {
                    console.log(`[CRON_COMPENSATION] Compensation ${comp.id} has expired. Reverting user ${comp.userId}...`);
                    ...
                    // Keep active compensations that haven't expired yet
                    if (comp.status === 'active') {
                        const timeRemaining = new Date(comp.endDate) - now;
                        const minutesRemaining = Math.floor(timeRemaining / 60000);
                        console.log(`[CRON_COMPENSATION] Compensation ${comp.id} still active. Expires in ${minutesRemaining} minutes`);
                    }
                    compensationsToKeep.push(comp);
                }
            }
        ...
        console.log("[CRON_COMPENSATION] Task started successfully!");
    } else {
        console.log("[CRON_COMPENSATION] Compensation revert task is disabled or has an invalid schedule.");
    }
}
