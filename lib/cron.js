 ...
 * Mencegah error saat WebSocket sudah mati tapi global.conn masih ada
...
async function safeSendMessage(jid, message, options = {}) {
    ...
    if (global.whatsappConnectionState !== 'open') {
        return { 
            success: false, 
            error: `WhatsApp not connected (state: ${global.whatsappConnectionState})`,
            shouldStop: true // Indikator untuk stop loop pengiriman
        };
    }
    
    // Cek apakah connection object tersedia
    ...
    try {
        await global.conn.sendMessage(jid, message, options);
        return { success: true };
    } catch (error) {
                ...
                for (let user of global.users) {
                    ...
                    if (!whitelistedProfile.includes(userProfile) && !user.paid) {
                        ...
                        if (!global.whatsappConnectionState || global.whatsappConnectionState !== 'open') {
                            console.error(`[CRON_REMINDER_ERROR] WhatsApp not connected (state: ${global.whatsappConnectionState})! Cannot send messages.`);
                            break;
                        }

                        if (user.phone_number && global.whatsappConnectionState === 'open' && global.conn) {
            ...
            for (const comp of currentCompensationsData) {
                if (comp.status === 'active' && new Date(comp.endDate) <= now) {
                    ...
                    try {
                        ...
                        if (currentCronConfig.status_message_compensation_reverted === true && templatesCache.notificationTemplates['compensation_reverted'] && global.whatsappConnectionState === 'open' && global.conn) {
            ...
            for (const req of global.speed_requests) {
                ...
                if (expirationDate <= now) {
                    ...
                    try {
                        ...
                        if (currentCronConfig.status_message_sod_reverted === true && templatesCache.notificationTemplates['speed_on_demand_reverted'] && global.whatsappConnectionState === 'open' && global.conn) {
                            if (userToRevert.phone_number && userToRevert.phone_number.trim() !== "") {
                                const dataPesan = {
                                    nama_pelanggan: userToRevert.name,
...
/**
 * Initialize Telegram Backup Task
 * Default: Setiap jam 4 pagi (0 4 * * *)
 */
