                    ...
                    if (redamanInt < rxTolerance) {
                        ...
                        for (const account of global.accounts) {
                            if (account.phone_number && account.phone_number.length > 0 && !account.phone_number.startsWith("0")) {
                                ...
                                try {
                                    if (global.conn) {
                                        // Use skipDuplicateCheck: true to allow multiple redaman alerts for different devices
                                        await global.conn.sendMessage(targetJid, { text: notificationText }, { skipDuplicateCheck: true });
