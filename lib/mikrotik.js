        ...
        .then(stdout => {
            try {
                ...
                if (result.status === 'error') {
                    // Throw error if PHP script returned error status
                    throw new Error(result.message || 'Gagal menambahkan PPPoE user.');
                ...
                if (e instanceof Error && e.message.includes('Gagal')) {
                    throw e;
                }
                // If parsing failed, try to extract error from string
                console.error(`[MikrotikJS_ERROR] Failed to parse JSON from adduserpppoe.php: ${e.message}. Raw output:`, stdout);
                // Check if stdout contains error message
                if (stdout.includes('sudah ada') || stdout.includes('already')) {
                    throw new Error('Akun PPPoE dengan nama yang sama sudah ada di MikroTik.');
                }
                throw new Error(`Gagal menambahkan PPPoE user: ${e.message || stdout}`);
            }
        });
}
...
/**
 * Check if PPPoE username already exists in MikroTik
 * @param {string} username - PPPoE username to check
 * @returns {Promise<boolean>} - True if exists, false if available
 */
function checkPPPoEUserExists(username) {
    if (!username || typeof username !== 'string') {
        return Promise.reject(new Error('Username tidak valid'));
    }
    
    return executePHPCommand('check_pppoe_username_exists', username)
        .then(stdout => {
            try {
                const result = JSON.parse(stdout);
                if (result.status === 'error') {
                    throw new Error(result.message || 'Gagal mengecek username');
                } else if (result.status === 'success') {
                    return result.exists === true;
                } else {
                    throw new Error('Respons tidak valid dari skrip pengecekan username');
                }
            } catch (e) {
                if (e instanceof Error && e.message.includes('Gagal')) {
                    throw e;
                }
                console.error(`[MikrotikJS_ERROR] Failed to parse JSON from check_pppoe_username_exists.php: ${e.message}. Raw output:`, stdout);
                throw new Error(`Gagal mengecek username: ${e.message || stdout}`);
            }
        });
}
 ...
 * @returns {Promise<Array>} Array of PPPoE secrets with name, password, profile, comment, disabled
 */
function getAllPPPoESecrets() {
    return executePHPCommand('get_all_pppoe_secrets')
        .then(stdout => {
            try {
                const result = JSON.parse(stdout);
                if (result.status === 'error') {
                    throw new Error(result.message || 'Gagal mengambil data PPPoE secrets');
                }
                return result.data || [];
            } catch (e) {
                if (e instanceof Error && e.message.includes('Gagal')) {
                ...
                console.error(`[MikrotikJS_ERROR] Failed to parse JSON from get_all_pppoe_secrets.php: ${e.message}. Raw output:`, stdout);
                throw new Error(`Gagal mengambil data PPPoE secrets: ${e.message || stdout}`);
            }
        });
}
