...
class TemplateManager {
     ...
     */
    loadAllTemplates() {
        console.log('[TemplateManager] Loading all templates...');
        let totalLoaded = 0;
        ...
        this.templateFiles.forEach(file => {
            ...
            if (fs.existsSync(filePath)) {
                try {
                    ...
                    totalLoaded += count;
                    console.log(`[TemplateManager] Loaded ${count} templates from ${file}`);
                } catch (error) {
                    console.error(`[TemplateManager] Error loading ${file}:`, error.message);
                }
            } else {
                console.warn(`[TemplateManager] Template file not found: ${file}`);
            }
        });
        
        console.log(`[TemplateManager] Total templates loaded: ${totalLoaded}`);
    }
