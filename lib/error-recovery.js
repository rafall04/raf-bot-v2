...
class ErrorRecovery {
    ...
    async reconnectWhatsApp() {
        ...
        try {
            ...
            if (global.raf) {
                try {
                    // Hanya end connection, jangan logout
                    if (global.raf.ws && global.raf.ws.close) {
                        global.raf.ws.close();
                    }
                } catch (e) {
                    console.log('[ERROR_RECOVERY] Error closing existing connection:', e.message);
                }
            }
            ...
            // Reconnect using global connect function
            if (global.connect && typeof global.connect === 'function') {
                await global.connect();
                console.log('[ERROR_RECOVERY] WhatsApp reconnection initiated');
                return true;
            } else if (global.startBot && typeof global.startBot === 'function') {
                await global.startBot();
                console.log('[ERROR_RECOVERY] WhatsApp reconnection initiated via startBot');
                return true;
            }
            
            console.log('[ERROR_RECOVERY] No reconnect function available');
            return false;
        } catch (error) {
