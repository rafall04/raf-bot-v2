...
function initializeDatabase() {
    return new Promise((resolve, reject) => {
        ...
        // NOTE: users are loaded from SQLite database, NOT from users.json!
        ...
        // Use database.sqlite in root directory, not in database folder
        const dbPath = path.join(__dirname, '..', 'database.sqlite');
        console.log("[DB_INIT] Using database at:", dbPath);
        const db = new sqlite3.Database(dbPath, (err) => {
            ...
            db.serialize(() => {
                // Skip CREATE TABLE - table already exists with correct schema
                ...
                db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='users'", (err, row) => {
                    ...
                    if (!row) {
                        console.error('Users table does not exist! Please run database migration.');
                        return reject(new Error('Users table not found'));
                    }
                    
                    console.log('[DB_INIT] Users table exists, loading data...');
